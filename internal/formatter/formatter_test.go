...
func sampleEvent() *pipeline.SNMPEvent {
	return &pipeline.SNMPEvent{
		...
		DeviceHostname: "core-router-01",
		DeviceType:     "router",
		DeviceVendor:   "Cisco",
		DeviceLocation: "DC-Tashkent",
		...
		Variables: []pipeline.Variable{
			{OID: "1.3.6.1.2.1.2.2.1.8.1", OIDName: "ifOperStatus.1", Value: 2, Type: "Integer"},
			{OID: "1.3.6.1.2.1.2.2.1.2.1", OIDName: "ifDescr.1", Value: "GigabitEthernet0/1", Type: "OctetString"},
		},
...
func TestCEFEscaping(t *testing.T) {
	f := NewCEFFormatter()
	event := sampleEvent()
	event.ValueStr = "value=with|special\\chars"
...
func TestJSONFormat(t *testing.T) {
	...
	if parsed["device_ip"] != "192.168.1.1" {
		t.Errorf("device_ip: want %q, got %v", "192.168.1.1", parsed["device_ip"])
	}
	if parsed["oid"] != "1.3.6.1.6.3.1.1.5.3" {
		t.Errorf("oid: want %q, got %v", "1.3.6.1.6.3.1.1.5.3", parsed["oid"])
	}
}
...
func TestJSONNoSensitiveData(t *testing.T) {
	f := NewJSONFormatter(false)
	event := sampleEvent()
	event.CustomFields = map[string]string{"note": "public-info"}

	result, _ := f.Format(event)

	if strings.Contains(result, "super-secret") {
		t.Error("JSON output should NOT contain sensitive data")
	}
}
...
func TestSyslogFormat(t *testing.T) {
	...
	// facility=16, severity=3(high) → 16*8+3 = 131
...
func TestSyslogFallbackHostname(t *testing.T) {
	f := NewSyslogFormatter()
	event := sampleEvent()
	event.DeviceHostname = "" // No hostname
